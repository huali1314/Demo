...
cc.Class({
    ...
    limitPlayerPos:function(){
        var playerPos_x = this.sprite.getPositionX()
        var playerPos_y = this.sprite.getPositionY()
        var winSize = cc.director.getWinSize()
        var spriteSize = this.sprite.getContentSize()
        var leftBorder = -winSize.width/2 + spriteSize.width/2
        var rightBorder = winSize.width/2 - spriteSize.width/2 
        var topBorder = winSize.height/2 - spriteSize.height/2 
        var bottomBorder =  -winSize.height/2 + spriteSize.height/2

        if (playerPos_x < leftBorder ||
            playerPos_x > rightBorder ){
            this.sprite.setPositionX((playerPos_x > 0)?(winSize.width/2 - spriteSize.width/2):(-winSize.width/2 + spriteSize.width/2))
        }
        if (playerPos_y < bottomBorder||
            playerPos_y > topBorder ){
            this.sprite.setPositionY((playerPos_y > 0)?(winSize.height/2 - spriteSize.height/2):(-winSize.height/2 + spriteSize.height/2))
        }
    },
